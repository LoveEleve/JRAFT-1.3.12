...
**源码**：`HashedWheelTimer.java:649-652`

```java
private static final class HashedWheelBucket {
    private HashedWheelTimeout head;  // 链表头（HashedWheelTimer.java:651）
    private HashedWheelTimeout tail;  // 链表尾（HashedWheelTimer.java:652）
}
```
...
**源码**：`HashedWheelTimer.java:522-555`

| 字段 | 类型 | 作用 | 源码行 |
|------|------|------|--------|
| `timer` | `HashedWheelTimer` | 所属定时器（用于取消时访问 cancelledTimeouts） | `HashedWheelTimer.java:533` |
| `task` | `TimerTask` | 任务体 | `HashedWheelTimer.java:534` |
| `deadline` | `long` | 到期时间（相对于 startTime 的纳秒偏移） | `HashedWheelTimer.java:535` |
| `state` | `volatile int` | 状态：0=INIT, 1=CANCELLED, 2=EXPIRED | `HashedWheelTimer.java:539` |
| `remainingRounds` | `long` | 剩余圈数（多圈时间轮的关键字段） | `HashedWheelTimer.java:542` |
| `next` / `prev` | `HashedWheelTimeout` | 双向链表指针 | `HashedWheelTimer.java:546-547` |
| `bucket` | `HashedWheelBucket` | 所在的 Bucket（用于 remove 时直接操作） | `HashedWheelTimer.java:550` |
...
**源码**：`HashedWheelTimer.java:445-462`
...
```java
// HashedWheelTimer.java:445-462（processCancelledTasks）
...
**源码**：`HashedWheelTimer.java:657-670`

**分支穷举清单**：

| 条件 | 结果 | 源码行 |
|------|------|-----|
| □ head == null | head = tail = timeout（链表为空，直接设为头尾） | `HashedWheelTimer.java:661-662` |
| □ head != null | tail.next = timeout; timeout.prev = tail; tail = timeout（追加到尾部） | `HashedWheelTimer.java:663-666` |

```java
// HashedWheelTimer.java:657-670（addTimeout）
...
**源码**：`HashedWheelTimer.java:696-720`

**分支穷举清单**：

| 条件 | 结果 | 源码行 |
|------|------|-----|
| □ timeout.prev != null | prev.next = next | `HashedWheelTimer.java:699` |
| □ timeout.next != null | next.prev = timeout.prev | `HashedWheelTimer.java:702` |
| □ timeout == head && timeout == tail | head=null; tail=null（链表只有一个节点） | `HashedWheelTimer.java:705-707` |
| □ timeout == head && timeout != tail | head = next（删除的是头节点） | `HashedWheelTimer.java:708-710` |
| □ timeout == tail（且不是 head） | tail = timeout.prev（删除的是尾节点） | `HashedWheelTimer.java:711-713` |
| □ 清空引用 | prev=null; next=null; bucket=null（帮助 GC） | `HashedWheelTimer.java:715-717` |
| □ 减少计数 | timer.pendingTimeouts.decrementAndGet() | `HashedWheelTimer.java:718` |

```java
// HashedWheelTimer.java:696-720（HashedWheelBucket.remove）
...
**源码**：`DefaultRaftTimerFactory.java:46-70`

```java
// DefaultRaftTimerFactory.java:46-70
...
```java
// DefaultRaftTimerFactory.java:158-165
...
```java
// DefaultRaftTimerFactory.java:99-102
...
```java
// DefaultRaftTimerFactory.java:148-155（SharedRef.getRef()）
...
| 条件 | 结果 | 源码行 |
|------|------|-----|
| □ shared == null 或 shared.isShutdown() | create() 重建 Shared 实例 | `DefaultRaftTimerFactory.java:150-152` |
| □ shared 正常 | shared.getRef()（引用计数 +1） | `DefaultRaftTimerFactory.java:154` |
...
**原因**：`HashedWheelTimer` 有一个全局计数器 `instanceCounter`，超过 256 个实例时打印 ERROR 日志（`HashedWheelTimer.java:55`）。
