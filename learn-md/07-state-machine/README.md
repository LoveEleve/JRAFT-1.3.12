...
```java
// FSMCallerImpl.java 第 148-165 行
...
```java
// FSMCallerImpl.java 第 103-120 行
...
**`resetFirstIndex()` 的前置检查**（`ClosureQueueImpl.java:97-104`）：
```java
...
```java
// FSMCallerImpl.java 第 370-430 行
...
对照源码（`FSMCallerImpl.java:330-345`）：
```java
case SNAPSHOT_SAVE:
...
原因：`SNAPSHOT_SAVE` 是在私有方法 `onSnapshotSaveSync(ApplyTask)` 内部（`FSMCallerImpl.java:435-440`）调用 `passByStatus`，而 `SNAPSHOT_LOAD` 是在 `runApplyTask` 的 switch-case 中直接检查。两者都有 `passByStatus` 保护，只是检查位置不同。

### 3.3 enqueueTask() 与 shutdown() 的协调

```java
// FSMCallerImpl.java 第 225-232 行
...
```java
// FSMCallerImpl.java 第 196-210 行
...
```java
// FSMCallerImpl.java 第 340-355 行
...
```java
// FSMCallerImpl.java 第 320-332 行
...
```java
// FSMCallerImpl.java 第 470-530 行
...
```java
// FSMCallerImpl.java 第 542-558 行
...
```java
// FSMCallerImpl.java 第 560-610 行
...
```java
// FSMCallerImpl.java 第 620-680 行
...
```java
// ClosureQueueImpl.java 第 115-145 行
...
```java
// ClosureQueueImpl.java 第 70-90 行
public void clear() {
...
```java
// FSMCallerImpl.java 第 730-745 行
...
1. **`onApply` 为什么是批量的？**
   ...
   - 源码：`FSMCallerImpl.java:310-325`（`runApplyTask` 中的 `maxCommittedIndex` 合并逻辑）
...
2. **Leader 和 Follower 的 `onApply` 有什么区别？**
   ...
   - 源码：`IteratorImpl.java:125-130`（`done()` 方法）
...
- ✅ **Closure 对应关系**：`IteratorImpl.done()` 通过 `currentIndex - firstClosureIndex` 索引 `closures` 列表（`IteratorImpl.java:125-130`）
- ✅ **错误后停止**：`doCommitted()` 开头 `!this.error.getStatus().isOk()` 检查（`FSMCallerImpl.java:392-394`）
